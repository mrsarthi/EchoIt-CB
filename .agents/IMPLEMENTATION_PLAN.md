...
| **Q13** | Repo name | **Done.** Development in `EchoIt-CB`; **releases publish to `EchoIt-Messenger`** — a **deliberate split**, decided 2026-08-23, not an oversight to reconcile. The updater endpoint is compiled into every build, so it must be right before the first release or installs poll a repo that never gets one. The publishing repo must stay **public**: the check is an unauthenticated GET. Runbook in `.agents/RELEASING.md` |
